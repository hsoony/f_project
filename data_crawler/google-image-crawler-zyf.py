...
# https://github.com/yansfil/google-image-crawler-zyf.git
# Import Libraries
...
from urllib.request import urlopen, urlopen,URLError, HTTPError, Request
...
CONFIGS[u'num_downloads_for_each_class'] = 10
...
if osp.exists(config_file):
    ...
    for k, v in CONFIGS_loaded.items():
        if k in CONFIGS:
            CONFIGS[k] = v
...
def google_get_query_url(keyword, file_type, cdr):
    ...
    if isinstance(keyword, str):
        keyword = keyword.encode('utf-8')
    if isinstance(keyword, (int, float, complex)):
...
for class_name, search_keywords in CONFIGS[u'search_keywords_dict'].items():
    ...
    while True:
        ...
        while j < len(search_keywords):
            ...
            if len(new_items) > CONFIGS[u'num_downloads_for_each_class']:
                new_items = new_items[:CONFIGS[u'num_downloads_for_each_class']]
            for url in new_items:
                ...
                if real_url and real_url not in items:
                    ...
                    fp_all_urls.write(str(real_url) + '\t' + save_name + "\n")
                    fp_urls.write(str(real_url) + '\t' + save_name + "\n")
